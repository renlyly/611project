...
A: I try this problem but all of the row have more than 1000 row, the filtered data remain the same. I prefer to do this problem with Remove rows with less than 2000 total counts
...
data['row_sum'] = numeric_cols.sum(axis=1)

filtered_data = data[data['row_sum'] >= 2000]

filtered_data = filtered_data.drop(columns=['row_sum'])

filtered_data.to_csv('filtered_episode_word_counts.csv', index=False)

print(f"Filtered data saved. Total number of rows after filtering: {len(filtered_data)}")
